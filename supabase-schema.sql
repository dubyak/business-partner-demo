-- =====================================================
-- Business Partner AI Demo - Supabase Database Schema
-- =====================================================
...
-- =====================================================
-- Extends Supabase Auth users with business-specific info
...
CREATE TABLE conversations (
    ...
    title TEXT, -- Optional: first message preview or custom title
...
CREATE TABLE messages (
    ...
    content JSONB NOT NULL, -- Supports text, images, and structured content
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    metadata JSONB -- Optional: token usage, model version, etc.
);

-- Enable RLS
ALTER TABLE messages ENABLE ROW LEVEL SECURITY;

-- Policies for messages (access through conversation ownership)
...
-- =====================================================
-- Detailed business information including financials
...
-- =====================================================
-- Tracks loan offers, amounts, terms, and status
CREATE TABLE loan_applications (
    ...
    interest_rate DECIMAL(5, 2), -- e.g., 5.25 for 5.25%
    status TEXT DEFAULT 'offered' CHECK (status IN ('offered', 'accepted', 'rejected', 'under_review', 'approved', 'declined')),
    offer_details JSONB, -- Additional structured offer information
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- =====================================================
-- Stores results from business photo analysis (vision AI)
CREATE TABLE photo_analyses (
    ...
    photo_data TEXT, -- Base64 if stored inline (consider storage for production)
    ...
    insights JSONB, -- Structured insights from vision analysis
    coaching_tips JSONB, -- Actionable recommendations
...
-- Function to update conversation last_message_at when messages are inserted
...
-- RLS for view (inherits from conversations)
ALTER VIEW conversation_summaries SET (security_invoker = true);

-- =====================================================
-- 9. STORAGE BUCKET (for photo uploads)
-- =====================================================
-- Note: This needs to be done via Supabase UI or Storage API
-- Instructions:
-- 1. Go to Storage in Supabase Dashboard
-- 2. Create a bucket named "business-photos"
...
-- 1. Check Tables tab in Supabase Dashboard
-- 2. Verify RLS is enabled on all tables (should see shield icon)
-- 3. Test with a sample insert using the authenticated user
-- =====================================================

-- Sample query to test (replace with your user_id after signup)
