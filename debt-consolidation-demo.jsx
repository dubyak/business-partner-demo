...
// API Configuration - routes through Railway backend
...
export default function DebtConsolidationDemo() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
      {/* Phone Frame - TALA Design System */}
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-200 relative">
        {/* Status Bar */}
        ...
        {/* Header - TALA Design: White background with logo */}
        <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
          <div className="text-xl font-bold text-gray-800 tracking-tight">TALA</div>
          <div className="flex-1"></div>
          <div className="flex items-center gap-2">
            <button 
              onClick={handleLanguageToggle}
              className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-md transition-colors"
            >
              {t.langToggle}
            </button>
            <button 
        ...
        <div className="h-96 overflow-y-auto p-4 space-y-3 bg-white">
          {messages.map((msg, idx) => (
            ...
            >
              <div className={`max-w-xs ${msg.role === 'user' ? 'order-2' : ''}`}>
                {msg.image && (
                  <div className="mb-1 rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                ...
                <div 
                  className={`px-4 py-2.5 rounded-xl text-sm leading-relaxed ${
                    msg.role === 'user' 
                      ...
                      : 'bg-white text-gray-700 rounded-bl-sm shadow-sm border border-gray-200'
                  }`}
                >
                  {msg.content}
                </div>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-white px-4 py-3 rounded-xl rounded-bl-sm shadow-sm border border-gray-200">
        ...
        {/* Input Area - TALA Design */}
        <div className="border-t border-gray-200 bg-white p-3">
          <form onSubmit={handleSubmit} className="flex gap-2 items-end">
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="w-10 h-10 rounded-full bg-white hover:bg-gray-50 border border-gray-200 flex items-center justify-center text-[#4ECDC4] transition-colors flex-shrink-0"
            >
            ...
            <input
              ...
              className="flex-1 bg-white border border-gray-200 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#4ECDC4] transition-all text-gray-700"
            />
            <button
              type="submit"
              disabled={!input.trim() && !isLoading}
              className="w-10 h-10 rounded-full bg-[#FF6B35] hover:bg-[#E55A2B] disabled:bg-gray-300 flex items-center justify-center text-white transition-colors flex-shrink-0"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Editable Demo Controls - TALA Card Design */}
      <div className="mt-6 max-w-sm w-full bg-white rounded-xl p-4 shadow-sm border border-gray-200">
        <h2 className="font-bold text-gray-800 mb-3 text-sm">{t.demoTitle}</h2>
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4ECDC4] bg-white text-gray-700"
            />
          </div>
          ...
          <div className="pt-2 flex items-center justify-between">
            <p className="text-xs text-gray-500">
              {t.offeredLimit}: <span className="font-semibold text-[#FF6B35]">$15,000 MXN max</span>
            </p>
            <button
              onClick={applySettingsAndRestart}
              className="px-4 py-2 bg-[#FF6B35] hover:bg-[#E55A2B] text-white text-xs font-bold rounded-md transition-colors"
            >
              {t.applyRestart}
            </button>
          </div>
          <p className="text-[#4ECDC4] text-xs mt-2">
            {t.uploadTip}
          </p>
        </div>
      </div>
